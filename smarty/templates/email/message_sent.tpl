...
{foreach $addressee.blocked as $a}{$a.first_name} {$a.last_name}, {if $a.former_message.is_public == 'yes'}{t escape=no}see message{/t} http://{$smarty.const.NJ_HOST}/?message={$a.former_message.id}{else}{t escape=no}the message was sent as a private one{/t}{/if}
...
website as well as as an eventual response of the contacted politicians.{/t}{else}{t escape=no 1=$smarty.const.NJ_TITLE}You have chosen a private message - it will not be published on the
